#!/usr/bin/env bash
#
# tools/sync-kiro.sh — gera a camada Kiro (.kiro/) a partir do canonico.
#
...
#
# Uso:
#   bash tools/sync-kiro.sh           # (re)gera .kiro/
#   bash tools/sync-kiro.sh --check   # exit 1 se .kiro/ commitado divergir do gerado
#
set -euo pipefail
...
if [ "$MODE" = "check" ]; then
  ...
  for p in steering skills; do
    if ! diff -r "$PACK_DIR/.kiro/$p" "$OUT/$p" > /dev/null 2>&1; then
      echo "DRIFT em .kiro/$p — rode: bash tools/sync-kiro.sh" >&2
