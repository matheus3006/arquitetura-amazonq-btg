...
|---|---|---|
| `.kiro/steering/negocio-style.md` (esta) | **REGRA** | Convenções + gate + hooks de negócio. |
...
Ao gerar HTML final, aplique também `.kiro/steering/frontend-style.md` (mesmo esqueleto, design-system e viewer da trilha técnica).
...
Idêntico ao da trilha técnica: siga `.kiro/steering/frontend-style.md` § 1 e o esqueleto de `architecture-style.md` § 3 (mesmo `shell`, `sidebar.js`, `tokens.css` + `components.css`, `diagram-viewer.js`). **O que muda são as seções de conteúdo**, definidas por cada prompt — não a estrutura.
