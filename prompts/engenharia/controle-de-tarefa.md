...
> próprio `docs/controle/<task-id>/` — não recarregue este arquivo durante a execução.
...
- O usuário pediu **qualquer entrega/artefato** — código, documento, spec, design, diagrama,
  ...
  normal) — e não há task ativa em `docs/controle/`. **Não é só para código.**
...
1. Derive o slug e o task-id (ver "Slug automático" acima) e anuncie-o. Crie
   `docs/controle/<task-id>/TASK.md` pelo template abaixo, com `fase: planejamento`.
...
Proibido no turno 1: editar qualquer arquivo fora de `docs/controle/<task-id>/`.
...
Leia `docs/controle/<task-id>/TASK.md` + `LEDGER.md`. Nada mais — nem PLANO, nem histórico de
...
- [ ] Nenhum arquivo fora de `docs/controle/` foi tocado?
