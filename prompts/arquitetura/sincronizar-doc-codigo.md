...
- `skills/arquitetura/grill-me/SKILL.md` — o **motor do grill**: interrogar uma pergunta por vez, descer cada ramo da árvore de decisão, resposta recomendada em cada uma, código antes de perguntar.
- `skills/arquitetura/human-architect-mindset/SKILL.md` — **controla as dimensões** do grill: QUAIS perguntas importam (invariantes, modos de falha, ordem temporal, escala, fronteiras de domínio). É o HAM que decide o que perguntar; o grill-me é como perguntar.
- `skills/backend/verification-before-completion/SKILL.md` — evidência (`arquivo:linha`) antes de afirmar; o que o código não revela vira `[a confirmar]`, não invenção.
- `prompts/arquitetura/gerador-adr.md` — encadeado na Fase D quando há decisão a registrar.
...
Siga **`skills/arquitetura/grill-me/SKILL.md`** com o **`human-architect-mindset`** no comando das dimensões. Para cada incerteza do inventário, na ordem de risco:
...
3. **Não existe** → **pergunte** se o usuário quer registrar a decisão. Se sim, **encadeie `prompts/arquitetura/gerador-adr.md`**, passando o contexto já capturado (problema, opções consideradas, trade-offs, o porquê do grill).
...
- Execução sob o protocolo de controle (`prompts/engenharia/controle-de-tarefa.md`): a doc e a(s) ADR(s) são o entregável.
...
> Na branch `feature/retry-estorno`. Siga `prompts/arquitetura/sincronizar-doc-codigo.md`:
...
## Referências
- Motor e dimensões do grill: `skills/arquitetura/grill-me/SKILL.md` + `skills/arquitetura/human-architect-mindset/SKILL.md`.
- Registro da decisão: `prompts/arquitetura/gerador-adr.md`.
- Fluxo de documentação do zero (complementar): `prompts/arquitetura/documentar-servico.md` → `completar-documentacao.md` → `grill-arquitetura.md`.
- Andaime do protocolo de grilling (ledger, rodada, anti-padrões): `prompts/engenharia/grill-plano.md` / `prompts/negocio/grill-negocio.md`.
