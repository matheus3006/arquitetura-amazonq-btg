...
>
> A lista de componentes abaixo reflete o **estado atual** do `ia/design-system/components.css`.
...
grep -rE "#[0-9a-fA-F]{3,8}\b" ia/templates/ | grep -v "(?:link)"

# Cores duplicadas no tokens.css
grep -E "^\s*--color" ia/design-system/tokens.css | sort

# Componentes definidos vs. usados
grep -E "^\.(\w+)" ia/design-system/components.css | awk -F'[ {.]' '{print $2}' | sort -u
```
...
> Use `ia/prompts/frontend/design-system-arquitetura.md`. Quero adicionar componente `.audit-trail` para mostrar histórico de mudanças em ADR. Auditar se já temos algo parecido.
...
## Referências
- Base: `ia/design-system/tokens.css`, `ia/design-system/components.css`.
