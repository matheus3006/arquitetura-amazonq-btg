...
> próprio `doc/controle/<task-id>/` — não recarregue este arquivo durante a execução.
...
- O usuário pediu **qualquer entrega/artefato** — código, documento, spec, design, diagrama,
  ...
  normal) — e não há task ativa em `doc/controle/`. **Não é só para código.**
...
1. Derive o slug e o task-id (ver "Slug automático" acima) e anuncie-o. Crie
   `doc/controle/<task-id>/TASK.md` pelo template abaixo, com `fase: planejamento`.
2. Crie o PLANO. Etapas no formato do `ia/prompts/engenharia/planejador-de-implementacao.md`
...
Proibido no turno 1: editar qualquer arquivo fora de `doc/controle/<task-id>/`.
...
Leia `doc/controle/<task-id>/TASK.md` + `LEDGER.md`. Nada mais — nem PLANO, nem histórico de
...
- [ ] Nenhum arquivo fora de `doc/controle/` foi tocado?
...
- Planos grandes (3+ arquivos): `ia/prompts/engenharia/planejador-de-implementacao.md`
- Estressar o plano antes da aprovação (opcional, recomendado em task de risco): `ia/prompts/engenharia/grill-plano.md`
- Disciplina do turno 2: `ia/prompts/engenharia/executor-de-plano.md` (+ `tdd-disciplinado.md` nas etapas de código)
- Bug no meio da execução: `ia/prompts/engenharia/depurador-sistematico.md`
- Hook de início de interação (abre a task sozinho, não bloqueia commit): Amazon Q `.amazonq/cli-agents/arquitetura.json` (`userPromptSubmit` → `.amazonq/hooks/controle-hook.sh`) e Kiro `.kiro/hooks/controle-prompt.kiro.hook` (`promptSubmit`)
