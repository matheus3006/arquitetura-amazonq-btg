#!/usr/bin/env bash
#
# ia/tools/sync-copilot.sh — gera a camada GitHub Copilot (.github/) a partir do canonico.
#
...
#   bash ia/tools/sync-copilot.sh --check   # exit 1 se .github/ commitado divergir do gerado
#
set -euo pipefail
...
# ── 3b) Biblioteca de skills importadas (ia/skills/<categoria>/<slug>/ → verbatim) ──
imported=0
...
if [ "$MODE" = "check" ]; then
  ...
  for p in copilot-instructions.md instructions prompts skills; do
    if ! diff -r "$PACK_DIR/.github/$p" "$OUT/$p" > /dev/null 2>&1; then
      echo "DRIFT em .github/$p — rode: bash ia/tools/sync-copilot.sh" >&2
