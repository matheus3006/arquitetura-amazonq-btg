#!/usr/bin/env bash
#
# ia/tools/sync-como-usar.sh — gera COMO-USAR.md a partir do COMO-USAR.html (canonico).
#
# Fonte canonica:  COMO-USAR.html (raiz do pack)
# Gerado (commitado, NUNCA editado a mao):  COMO-USAR.md (raiz do pack)
#
# Uso:
#   bash ia/tools/sync-como-usar.sh           # (re)gera COMO-USAR.md
#   bash ia/tools/sync-como-usar.sh --check   # exit 1 se o .md commitado divergir do gerado
#
set -euo pipefail

PACK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SRC="$PACK_DIR/ia/COMO-USAR.html"
DST="$PACK_DIR/ia/COMO-USAR.md"
...
if [ "$MODE" = "check" ]; then
  if diff -q "$DST" "$OUT" > /dev/null 2>&1; then
    ...
    echo "DRIFT em COMO-USAR.md — rode: bash ia/tools/sync-como-usar.sh" >&2
